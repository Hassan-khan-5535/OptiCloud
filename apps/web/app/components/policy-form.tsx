...
export function PolicyForm({ accounts }: { accounts: Account[] }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!Number.isFinite(costDollars) || costDollars < 0) { setState('error'); setMessage('maximum estimated savings must be a non-negative number.'); return; }
    setState('saving');
    setMessage('');
    try {
  ...
  return <form onSubmit={submit} className="space-y-5">
    ...
    <div className="rounded-lg royal-form-block border border-[#1a3554] bg-blue-950/25 p-4"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-blue-200">All conditions must match</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><div><label htmlFor="min-age-days" className="label">Age ≥ days</label><input id="min-age-days" type="number" min="0" step="1" value={minAgeDays} onChange={(event) => setMinAgeDays(event.target.value)} className="field" /></div><div><label htmlFor="max-monthly-cost" className="label">Estimated monthly savings ≤ USD</label><input id="max-monthly-cost" type="number" min="0" step="0.01" value={maxMonthlyCost} onChange={(event) => setMaxMonthlyCost(event.target.value)} className="field" /></div></div><p className="mt-3 text-[11px] leading-5 text-slate-500">Stored as explicit <code className="text-blue-100">all</code> conditions. Savings are converted to cents before persistence.</p></div>
    <label className="flex cursor-pointer items-center justify-between gap-4 rounded-lg border border-[#1a3554] bg-blue-950/20 px-3 py-3"><span><span className="block text-xs font-semibold text-slate-200">Live policy</span><span className="mt-1 block text-[11px] text-slate-500">Turn off to evaluate as dry-run only.</span></span><input type="checkbox" checked={active} onChange={(event) => setActive(event.target.checked)} className="h-4 w-4 accent-[#f5bd47]" /></label>
    <button type="submit" disabled={state === 'saving' || accounts.length === 0} className="w-full royal-button rounded-lg px-4 py-2.5 text-xs font-bold uppercase tracking-[0.12em] transition active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-50">{state === 'saving' ? 'Saving…' : active ? 'Create live policy' : 'Create dry-run policy'}</button>
    {message && <p role={state === 'error' ? 'alert' : 'status'} aria-live="polite" className={`text-xs ${state === 'saved' ? 'text-emerald-300' : 'text-rose-300'}`}>{message}</p>}
  </form>;
}
