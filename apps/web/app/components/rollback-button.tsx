...
export function RollbackButton({ remediationActionId }: { remediationActionId: string }) {
  ...
  if (state === 'queued') return <span role="status" aria-live="polite" className="inline-flex items-center gap-2 rounded-lg border border-emerald-400/25 bg-emerald-400/10 px-3 py-2 text-xs font-semibold text-emerald-300"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400" aria-hidden="true" />{message}</span>;
