...
function JsonBlock({ value }: { value: Record<string, unknown> }) {
  return <pre className="max-h-[420px] overflow-auto rounded-lg border border-[#1a3554] bg-[#050a12] p-4 font-mono text-xs leading-6 text-cyan-100/80">{JSON.stringify(value, null, 2)}</pre>;
}
...
export default async function FindingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return <div className="space-y-5"><Link href="/" className="royal-link text-xs">← Back to overview</Link><div className="rounded-xl border royal-panel border-rose-400/20 bg-rose-400/5 p-5 text-sm text-rose-200">Finding data unavailable. {error instanceof Error ? error.message : 'The API request failed'}.</div></div>;
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-5 royal-divider border-b pb-7 md:flex-row md:items-end md:justify-between">
        <div><Link href="/" className="royal-link text-xs font-medium">← Back to overview</Link><div className="mt-5 flex flex-wrap items-center gap-3"><span className="font-mono text-xs text-slate-500">FINDING / {finding.id.slice(0, 8)}</span><StatusBadge status={finding.status} /></div><h1 className="mt-3 text-3xl font-semibold tracking-tight text-white">{humanize(finding.findingType)}</h1><p className="mt-2 text-sm text-slate-400">{finding.resource.externalId} · {humanize(finding.resource.type)} · {finding.resource.region}</p></div>
        {canRollback && finding.remediationAction && <RollbackButton remediationActionId={finding.remediationAction.id} />}
      </div>

      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.8fr)]">
        <div className="space-y-5">
          <section className="royal-panel rounded-xl border p-5"><div className="flex items-center justify-between gap-3"><div><h2 className="text-sm font-semibold text-white">Evidence</h2><p className="mt-1 text-xs text-slate-500">Raw detector output from the waste finding</p></div><span className="rounded border border-slate-700 px-2 py-1 text-[10px] uppercase tracking-wider text-slate-500">JSON</span></div><div className="mt-5"><JsonBlock value={finding.evidence} /></div></section>
          <section className="royal-panel rounded-xl border p-5"><div className="flex items-center justify-between"><div><h2 className="text-sm font-semibold text-white">State transition history</h2><p className="mt-1 text-xs text-slate-500">Append-only entries from <code className="text-slate-300">audit_log</code></p></div><span className="font-mono text-xs text-slate-600">{finding.auditLog.length} events</span></div><div className="mt-6"><FindingTimeline entries={finding.auditLog} /></div></section>
        </div>
        <div className="space-y-5">
          <section className="royal-panel rounded-xl border p-5"><h2 className="text-sm font-semibold text-white">Cost model</h2><p className="mt-1 text-xs leading-5 text-slate-500">Why this finding is worth acting on</p><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-lg border border-[#1a3554] bg-blue-950/25 p-3"><p className="text-[10px] uppercase tracking-wider text-slate-600">Current monthly cost</p><p className="mt-2 font-mono text-lg font-semibold text-slate-100">{formatCurrency(finding.costModel.currentMonthlyCostCents)}</p></div><div className="rounded-lg border border-blue-400/25 bg-blue-950/35 p-3"><p className="text-[10px] uppercase tracking-wider text-blue-200/80">Projected savings</p><p className="mt-2 font-mono text-lg font-semibold text-blue-200">{formatCurrency(finding.costModel.projectedMonthlySavingsCents)}</p></div></div><p className="mt-4 text-xs leading-5 text-slate-400">{finding.costModel.explanation}</p></section>
