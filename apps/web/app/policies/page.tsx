...
export default async function PoliciesPage() {
  ...
  return <div className="space-y-8">
    ...
    {error ? <div className="rounded-xl border border-rose-400/20 bg-rose-400/5 p-5 text-sm text-rose-200">Policy data unavailable. {error}.</div> : <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
      <section className="overflow-hidden rounded-xl border border-slate-800/90 bg-[#0b1220]"><div className="flex items-center justify-between border-b border-slate-800/90 px-5 py-5"><div><h2 className="text-sm font-semibold text-white">Auto-approve policies</h2><p className="mt-1 text-xs text-slate-500">Live rules and inactive rules running in dry-run mode</p></div><span className="rounded-md border border-slate-700 bg-slate-900 px-2.5 py-1 font-mono text-[11px] text-slate-400">{policies.length} rules</span></div>{policies.length === 0 ? <div className="data-grid px-5 py-16 text-center"><p className="text-sm font-medium text-slate-300">No auto-approve policies.</p><p className="mt-2 text-xs text-slate-500">Create a dry-run policy first to trial its matches safely.</p></div> : <div className="divide-y divide-slate-800/80">{policies.map((policy) => <PolicyRow key={policy.id} policy={policy} />)}</div>}</section>
