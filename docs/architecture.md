...
## Stage 5 remediation execution engine

Stage 5 consumes the `execute-remediation` BullMQ jobs created by the Slack approval flow. `apps/worker/src/remediation/engine.ts` maps only the three MVP finding types to actions. An `unattached_volume` uses `delete_volume`, but first creates and persists a provider snapshot as the action’s rollback instruction; deletion is never attempted if snapshot creation fails. An `idle_load_balancer` uses `stop_load_balancer` only when the provider advertises a stopped state. AWS Elastic Load Balancing does not provide that state, so the AWS adapter reports unsupported capability and the engine records manual review rather than deleting the load balancer. An `underutilized_rds` always uses `resize_instance` down one known tier. RDS receives gentler treatment than EBS because resizing can preserve the stateful service and be reversed, whereas stopping or deleting a database risks availability and data loss.

All mutating operations go through `CloudRemediationProvider` in `packages/cloud-adapters/src/remediation.ts`. The AWS SDK implementation is isolated in `aws-remediation.ts`; the worker’s tests use `MockCloudRemediationProvider`. `RedisRateLimiter` keys one-second request buckets by provider and cloud account, defaults to five requests per second, and supports provider-specific `AWS_REQUESTS_PER_SECOND` and `GCP_REQUESTS_PER_SECOND` settings.

Before any provider call, the engine requires an approved finding, creates or reuses the unique remediation action, and transitions both finding and action to executing through the Stage 2 audited state machine. On success, both become completed. On provider failure, the exact provider error is included in the audit reason and both become failed, so a job is never left in executing. BullMQ jobs use three attempts with exponential backoff; the worker throws failed results so BullMQ actually retries, and subsequent attempts reuse the same action and any already-created EBS snapshot.

`rollbackRemediation(remediationActionId)` validates the persisted JSONB rollback instruction before making a provider call. It supports restoring an EBS volume from its pre-delete snapshot, starting a provider-supported load balancer, or resizing RDS back to its recorded previous class. The API endpoint `POST /api/remediations/:id/rollback` enqueues a deterministic rollback job for the worker; it does not execute cloud mutations in the API process.

## Stage 5 safety judgment

The only action I would consider for narrowly scoped auto-approval in a real production account is an RDS resize, and even that requires maintenance-window controls, a verified one-tier mapping, replica/availability checks, and a provider-specific rollback plan. I would not auto-approve EBS volume deletion in production by default: the snapshot-first guard materially reduces risk, but deletion remains destructive, snapshots incur cost, and restore correctness depends on availability-zone and attachment assumptions. I would not auto-approve load-balancer stopping; for AWS it is unsupported and the correct behavior is manual review, while a provider that supports stopping would still need traffic, health-check, and dependency validation. In general, human approval should remain the default until live-provider integration tests and operational guardrails prove these assumptions for each account.
